...
interface Student { id: string; name: string; attendance: string; regular: string; }
...
function parseCSV(text: string): Student[] {
  ...
  const start = lines.length > 0 && /^\s*(学号|id|name|姓名|出勤|考勤|平时|attendance|regular)/i.test(lines[0]) ? 1 : 0;
  ...
  const renderTable = () => {
    ...
    return (
      <>
        ...
        <table>
          <thead><tr><th>#</th><th>学号</th><th>姓名</th><th>考勤成绩</th><th>平时成绩</th></tr></thead>
          <tbody>
            {students.map((s, i) => (
              <tr key={i}>
  ...
  const renderUpload = () => {
    if (!authed) {
      return (
        <div style={{border:"2px solid #ccc",borderRadius:"8px",padding:"1.5em",background:"#fafafa",marginTop:"1.2em"}}>
          <p style={{fontSize:"0.9rem",marginBottom:"0.8em"}}><strong>🔒 需验证后方可修改学生数据</strong></p>
          ...
          <button onClick={login} style={{padding:"0.5em 2em",background:"#2f4f4f",color:"#f5deb3",border:"1px solid #000",borderRadius:"4px",cursor:"pointer",fontWeight:"bold"}}>验证</button>
    ...
    return (
      <div style={{border:"2px dashed #ccc",borderRadius:"8px",padding:"1.2em",background:"#fafafa",marginTop:"1.2em"}}>
        ...
        <p style={{fontSize:"0.78rem",color:"#888",marginBottom:"0.3em"}}>格式：学号,姓名,考勤成绩,平时成绩（考试成绩不录入，涉及隐私）</p>
